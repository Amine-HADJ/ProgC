...
int recois_numeros_calcule(char *data)
{
  ...
  if (strcmp(&op, "somme") == 0)
  {
    op = '+';
  }

  if (strcmp(&op, "moyenne") == 0)
  {
    op = '_';
  }
...
int recois_envoie_message(int client_socket_fd, char *data)
{
...

  {
    ...
    else if (strcmp(code, "calcule:") == 0)
    {
      ...
      float resultat = recois_numeros_calcule(data);

      char message[1024];
      sprintf(message, "%.2f", resultat);
...
int main()
{
  ...
  while (1)
  {
    ...
    printf("Nouvelle connexion cliente acceptée.\n");
